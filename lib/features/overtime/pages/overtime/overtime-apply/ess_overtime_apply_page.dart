...
class _ApplyOvertimeFormPageState extends State<ApplyOvertimePage> {
  // controllers obtained via Binding in routes (Get.lazyPut)
  ...
  String _calculateOvertimeDuration() {
    ...
    DateTime start = DateTime(now.year, now.month, now.day, _startTime!.hour, _startTime!.minute);
    DateTime end = DateTime(now.year, now.month, now.day, _endTime!.hour, _endTime!.minute);
  ...
  DateTime? get _endDate {
    ...
    return _isNextDay ? _selectedDate!.add(const Duration(days: 1)) : _selectedDate;
  }
  ...
  Future<void> _selectTime(bool isStart) async {
    ...
    if (picked != null) {
      final formatted = DateFormat('HH:mm').format(DateTime(0, 1, 1, picked.hour, picked.minute));
      setState(() {
        if (isStart) {
          _startTime = picked;
  ...
  Future<void> _showReasonPicker() async {
    // reasonController.reasons diharapkan List<EssReasonOT>
  ...
  void _goToConfirmation() {
    if (_formKey.currentState!.validate()) {
      if (_selectedDate == null || _startTime == null || _endTime == null || _selectedReason == null) {
        Get.snackbar(
          "Error",
      ...
      Get.to(() => ApplyOvertimeConfirmPage(
        date: _selectedDate!,
        endDate: _endDate,
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: Colors.blue[900]!, width: 2)),
    ...
    return Container(
      ...
      child: const Row(
        children: [
          ...
          SizedBox(width: 8),
          Expanded(
            child: Text('Lembur sampai keesokan hari', style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  Widget _buildRequestHeader(DateTime start, DateTime? end, bool isNextDay) {
    ...
    final endStr = isNextDay && end != null ? DateFormat('yyyy-MM-dd (E)').format(end) : null;
    ...
    return Container(
      ...
      child: Text(
        endStr != null ? "OT Request untuk tanggal $startStr → $endStr" : "OT Request untuk tanggal $startStr",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Ajukan Lembur", style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              if (_selectedDate != null) _buildRequestHeader(_selectedDate!, _endDate, _isNextDay),
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      _buildField(_dateController, "Tanggal Lembur", "Pilih Tanggal", Icons.calendar_today, _selectDate),
                      Row(
                        children: [
                          Expanded(child: _buildField(_startTimeController, "Waktu Mulai", "Mulai", Icons.access_time, () => _selectTime(true))),
                          const SizedBox(width: 10),
                          Expanded(child: _buildField(_endTimeController, "Waktu Selesai", "Selesai", Icons.access_time, () => _selectTime(false))),
                        ],
                      ),
                      if (_isNextDay) _buildNextDayWarning(),
                      _buildField(_reasonController, "Alasan Lembur", "Pilih alasan", Icons.arrow_drop_down, _showReasonPicker),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: const Text("Lanjut ke Konfirmasi", style: TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
            ],
          ),
